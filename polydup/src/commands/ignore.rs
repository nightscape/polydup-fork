...
fn find_duplicate_id_by_location(
    ...
    let file_ranges: Vec<FileRange> = files
        .iter()
        .map(|f| FileRange::parse(f).map_err(anyhow::Error::from))
...
fn handle_list_command(
    ...
    match format {
        OutputFormat::Json | OutputFormat::Sarif => {
            // SARIF format falls back to JSON for ignore list (SARIF is for scan results)
            let json = serde_json::to_string_pretty(&entries)
                .context("Failed to serialize ignore list")?;
            println!("{}", json);
        }
        OutputFormat::Text | OutputFormat::Vscode | OutputFormat::Compiler => {
            // IDE formats fall back to text for ignore list (they're for scan results)
            println!(
                "{}",
...
fn handle_remove_command(id: &str, manager: &mut dupe_core::IgnoreManager) -> Result<()> {
    ...
    if removed {
        ...
        // Check if the ID is ambiguous (matches multiple entries)
        let matches = manager.find_all_by_prefix(id);
        if matches.len() > 1 {
            println!(
                "{}",
                format!("✗ Ambiguous ID '{}' matches multiple entries:", id).red()
            );
            for matched_id in &matches {
                println!("   {} {}", "•".bright_black(), matched_id.bright_blue());
            }
            println!();
            println!(
                "{}",
                "Provide a longer ID prefix to uniquely identify the entry.".yellow()
            );
            std::process::exit(1);
        } else {
            ...
            println!(
                "{}",
                "Run 'polydup ignore list' to see all ignored duplicates.".dimmed()
            );
            std::process::exit(1);
        }
    }
