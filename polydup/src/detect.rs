...
impl Environment {
    ...
    pub fn install_command(&self) -> &'static str {
        match self {
            Environment::NodeJs => "npm install -g polydup",
...
pub fn combined_exclude_patterns(environments: &[Environment]) -> Vec<String> {
    // Start with global patterns that should always be excluded
    let mut patterns: Vec<String> = vec!["**/.git/**".to_string(), "**/vendor/**".to_string()];

    // Add environment-specific patterns
    patterns.extend(environments.iter().flat_map(|env| env.exclude_patterns()));
...
mod tests {
    ...
    fn test_combined_exclude_patterns() {
        ...
        // Should contain global patterns
        assert!(
            patterns.contains(&"**/.git/**".to_string()),
            "Missing global pattern: .git"
        );
        assert!(
            patterns.contains(&"**/vendor/**".to_string()),
            "Missing global pattern: vendor"
        );

        // Should contain Node.js patterns
        assert!(patterns.contains(&"**/node_modules/**".to_string()));
        assert!(
            patterns.contains(&"**/coverage/**".to_string()),
            "Missing Node.js pattern: coverage"
        );
        assert!(
            patterns.contains(&"**/*.d.ts".to_string()),
            "Missing Node.js pattern: *.d.ts"
        );
