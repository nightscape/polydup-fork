...
pub fn run(mut config: ScanConfig) -> Result<()> {
    ...
    // Perform scan
    // In git-diff mode, scan only the changed files for performance (10-100x speedup)
    let scan_paths = if let Some(ref changed) = changed_files {
        changed.clone()
    } else {
        config.paths.clone()
    };
    let mut report = scanner
        .scan(scan_paths.clone())
        .map_err(|e| build_scan_error(e.into(), &scan_paths, &config))?;
    ...
    // Successful scan - return Ok regardless of duplicates found
    // Exit code 0: Scan completed successfully (with or without duplicates)
    // Non-zero exit codes are reserved for errors (invalid paths, parsing failures, etc.)
    Ok(())
}
...
fn generate_output(report: &dupe_core::Report, config: &ScanConfig) -> Result<String> {
    match config.format {
        ...
        OutputFormat::Text => {
            format_text_report(report, config.verbose, config.group_by.as_deref())
        }
    }
}
